...
if(results2||(results.attributes.typeOfUser.toString().toLowerCase()=="manager"||results.attributes.typeOfUser.toString().toLowerCase()=="admin")){
  ...
  const handleCellClick = useCallback(async (event) => {
    ...
    const queryNotas = new Moralis.Query("Qualifications");

    let res = await query.first();
    queryNotas.equalTo("teacherEmail", res.attributes.teacherEmail);

    let resNotas = await query.find();
    let count=0
    if(resNotas!==undefined){
      
    if(resNotas?.attributes?.classDuration){
      
    for (let i = 0; i < resNotas.length; i++) {
    count+=parseFloat(resNotas.attributes.classDuration)
    }
  }
 ...
 */}                    <Box sx={{ width:"100%",marginTop: 10, marginBottom: 10 }}>
            ...
            <Typography>{"Horas trabajadas "+workedHours}</Typography>
            {isModerator?  <LoadingButton
              fullWidth
              size="large"
              sx={{ mt: 3 }}
