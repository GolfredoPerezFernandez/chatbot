...
const columnsCourse = [
  ...
  { field: 'studentState', headerName: 'State', width: 200 },
...
var [courses,setCourses]=useState([])
  ...
  const query2 = new Moralis.Query("Administrators");
...
console.log(cursos.length)
    const query = new Moralis.Query("Administrators");
    ...
    let studiantes=[]
      for(let i=0;i<object.length;i++){
        ...
        studiantes=[...studiantes,{
          ...
          studentEmail:object[i].attributes.adminEmail,
          studentName:object[i].attributes.adminName,
          studentState:object[i].attributes.adminState,
...
async function handleStudent(){
  ...
  const Student= Moralis.Object.extend("Administrators")
  ...
  if(values.studentInstitute){
   ...
   return 
    ...
    if(values.studentName!==""){
      ...
      student.set("adminName",values.studentName)    
    
    } else{      
        setLoading(false)

      setError("Falta el nombre del admin")
    ...
    if(values.studentOcupacion!==""){
      ...
      student.set("adminOcupacion",values.studentOcupacion)    
    
    } else{
      setError("Falta la ocupacion del admin")
      setLoading(false)
    ...
    if(values.studentAddress!==""){
      ...
      student.set("adminAddress",values.studentAddress)    
    
    } else{
      setError("Falta la direccion del admin")
      setLoading(false)
    ...
    if(values.studentEmail!==""){
      ...
      student.set("adminEmail",values.studentEmail)    
    
    } else{
      setError("Falta el correo del admin")
      setLoading(false)
    ...
    if(values.studentInactivity!==""){
      ...
      student.set("adminInactivity",values.studentInactivity)    
    
    } else{
      student.set("adminInactivity",undefined)    
   
    }
    if(values.studentLastname!==""){

      student.set("adminLastname",values.studentLastname)          
    
    }  else{
      setError("Falta el apellido del admin")
      setLoading(false)
...
if(values.studentID!==""){
    ...
    student.set("adminID",values.studentID)
    
}else{
  setError("Falta la cedula del admin")
  setLoading(false)
...
if(valuesLenguage){
  student.set("adminLenguage",valuesLenguage)

  
}else{
  setError("Falta el lenguage del admin")
  setLoading(false)
...
if(values.studentGender){
  student.set("adminGender",values.studentGender)
 
}else{
  student.set("adminGender","Male")
  
}


if(values.studentPhone){
  student.set("adminPhone",values.studentPhone)

} else {
  setError("Falta el telefono del admin")
  setLoading(false)
...
if(values.studentAlergies){
  student.set("adminAlergies",values.studentAlergies)

  
}else{
  setError("Falta las alergias del admin")
  setLoading(false)
...
if(dateBirthday){
  student.set("adminBirthday",dateBirthday.toString())
 
}else{
  setError("Falta el cumpleaños del admin")
  setLoading(false)
...
if(values.studentComments){
  student.set("adminComments",values.studentComments)
 
}else{
  setError("Falta los comentarios del admin")
  setLoading(false)
...
if(values.studentDegree){
  student.set("adminDegree",values.studentDegree)
 
}else{
  setError("Falta el grado del admin")
  setLoading(false)
...
if(values.studentCity){
  student.set("adminCity",values.studentCity)
 
}else{
  setError("Falta la ciudad del admin")
  setLoading(false)

  return
}
if(values.studentState){
  student.set("adminState",values.studentState)
} else{
  student.set("adminState","Activo")

}
if(values.studentInstitute){
  student.set("adminInstitute",values.studentInstitute)
 
}else{
  setError("Falta el instituto del admin")
  setLoading(false)
