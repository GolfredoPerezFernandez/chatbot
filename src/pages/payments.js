    ...
    let studiantes=[]
      for(let i=0;i<object2.length;i++){
        ...
        studiantes=[...studiantes,{
          label:object2[i].attributes.studentName,
...
const handlePago =async ()=>{
  ...
  if(values.students==""){
    ...
    setError("Falta el students del curso")
    return
  }else{
    console.log(values.students)
    res.set("studentName",values.students)       
  ...
  if(values.courseId===""){
    ...
    res.set("courseId",res2.attributes.courseId)
...
if(values.bankName===""){
  ...
  course.set("bankName",values.bankName)       
    ...
    if(!values.students){
      ...
      setError("Falta el estudiante")
      return
    }else{

      course.set("studentId",values.students)       
    ...
    if(values.courseId===""){
      ...
      const query2 = new Moralis.Query("Courses");
      query2.equalTo("uid",values.courseId)
      let res2=await query2.first()
      course.set("courseId",values.courseId)    
      console.log(res2)
if(res2){
  course.set("course",res2.attributes.courseName)
    ...
    if(values.courseId===""){
      ...
      setError("Falta el curso del pago")
      return 
   }else {   

    console.log("values.courseId "+values.courseId)
...
console.log(res.attributes.paymentImage)
    setValues({name:res.attributes.name,avatar:res.attributes.avatar,paymentType:res.attributes.paymentType,paymentType:res.attributes.paymentType,referencia:res.attributes.referencia,studentId:res.attributes.studentId})  
...
query.equalTo("uid",parseInt(event.target.value))
let res= await query.first()
